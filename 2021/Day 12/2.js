function countPaths(data) {
	...
	for(const cave of pathsMap.start) {
		followPath('start', cave, [], false);
	}

	function followPath(caveFrom, caveCurrent, path, visitedSmallCaveOnce) {
		...
		for(const cave of pathsMap[caveCurrent]) {
			...
			else if(/[a-z]/.test(cave) && path.includes(cave)) {
				if(visitedSmallCaveOnce) {
					continue;
				}
				else {
					followPath(caveCurrent, cave, path, true);
				}
			}
			else {
				followPath(caveCurrent, cave, path, visitedSmallCaveOnce);
			}
		}
	}
